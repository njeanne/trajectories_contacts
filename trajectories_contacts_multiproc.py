...
def hydrogen_bonds(inspected_traj, data, atoms_dist, angle):
    ...
    # get the distances
    logging.info(f"\tGet the hydrogen bonds distances, please be patient..")
    logging.info(f"process: {h_bonds}")
    distances = pt.pmap(pt.distance, inspected_traj, h_bonds.get_amber_mask()[0])
    logging.info(f"process finished")
    logging.info(f"process: {len(distances)}\n{distances}")
